...
class SoundManager(private val context: Context) {
    ...
    private fun vibrateLight() { vibrate(30L, VibrationEffect.EFFECT_TICK) }
    private fun vibrateMedium() { vibrate(50L, VibrationEffect.EFFECT_CLICK) }
    private fun vibrateHeavy() { vibrate(150L, VibrationEffect.EFFECT_HEAVY_CLICK) }

    private fun vibrate(durationMs: Long, effectId: Int) {
        vibrator?.let { v ->
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                try { v.vibrate(VibrationEffect.createPredefined(effectId)) }
                catch (_: Exception) { v.vibrate(VibrationEffect.createOneShot(durationMs, VibrationEffect.DEFAULT_AMPLITUDE)) }
            } else {
